...
def encode_texts_batch(
    ...
    if show_progress:
        iterator = tqdm(
            range(0, len(processed_texts), batch_size), desc="Generating embeddings"
        )
    else:
...
def process_student_responses(
    ...
    logger.info(
        f"Found {len(valid_responses)} valid responses out of {len(df)} total rows"
    )
...
def save_embeddings(
    embeddings: np.ndarray, output_dir: str, base_name: str = "embeddings"
) -> None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate embeddings from student responses"
    )
    parser.add_argument("--input", default="data/train.csv", help="Input CSV file path")
    parser.add_argument(
        "--output", default="data/embeddings", help="Output directory for embeddings"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--column",
        default="StudentExplanation",
        help="Column name containing student responses",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--list-models", action="store_true", help="List recommended models and exit"
    )
    ...
    try:
        ...
        embeddings, id_to_index = process_student_responses(
            df=df, model=model, column_name=args.column, batch_size=args.batch_size
        )
        ...
        # Save id mapping
        save_id_mapping(
            id_to_index, args.output, "student_id_mapping", args.mapping_format
        )
