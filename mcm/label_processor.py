...
def load_unique_labels(file_path: str = "data/train.csv") -> pd.DataFrame:
    ...
    try:
        ...
        unique_labels = (
            df.groupby(["Category", "Misconception"]).size().reset_index(name="count")
        )
        unique_labels = unique_labels.sort_values("count", ascending=False)

        logger.info(
            f"Found {len(unique_labels)} unique Category:Misconception combinations"
        )
...
def parse_label(label: str) -> Dict[str, str]:
    ...
    if label.lower() in [
        ...
        return {"topic": "correctness", "misconception": label.strip().lower()}
    elif label.lower() in ["true_misconception", "false_misconception"]:
        return {"topic": "misconception", "misconception": label.strip().lower()}
    else:
        # Assume it's a misconception type
        return {"topic": "mathematics", "misconception": label.strip().lower()}
...
def save_labels_to_csv(
    processed_labels: List[Dict[str, Any]], output_path: str = "data/labels.csv"
) -> None:
...
def save_labels_to_json(
    processed_labels: List[Dict[str, Any]], output_path: str = "data/labels.json"
) -> None:
...
def get_label_statistics(processed_labels: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    stats = {
        ...
        "unique_misconceptions": len(
            set(label["misconception"] for label in processed_labels)
        ),
        "top_topics": {},
        "top_misconceptions": {},
    }
    ...
    for label in processed_labels:
        ...
        misconception_counts[misconception] = (
            misconception_counts.get(misconception, 0) + label["count"]
        )

    stats["top_misconceptions"] = dict(
        sorted(misconception_counts.items(), key=lambda x: x[1], reverse=True)[:10]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Process Category:Misconception labels"
    )
    parser.add_argument("--input", default="data/train.csv", help="Input CSV file path")
    parser.add_argument(
        "--output-csv", default="data/labels.csv", help="Output CSV file path"
    )
    parser.add_argument(
        "--output-json", default="data/labels.json", help="Output JSON file path"
    )
    parser.add_argument(
        "--show-stats", action="store_true", help="Show label statistics"
    )
    parser.add_argument(
        "--show-samples", action="store_true", help="Show sample parsed labels"
    )
    ...
    try:
        ...
        if args.show_samples:
            ...
            for i, label in enumerate(processed_labels[:5]):
                logger.info(
                    f"  {i+1}. {label['combined_label']} → topic: {label['topic']}, misconception: {label['misconception']}"
                )
